...
from wavestitch.data_utils import datasets, CyclicEncoder, Preprocessor
...
from wavestitch.models.sssd_s4_imputer import SSSDS4Imputer, SSSDS4Weaver, SSSDS4ImputerClassic
from wavestitch.models.timegan import TimeGAN
...
def resolve_model_columns(frame, preprocessor):
    """Return signal and metadata positions in the model's cleaned dataframe."""

    columns = frame.columns
    metadata_columns = list(preprocessor.hierarchical_features_cyclic)
    metadata_indices = columns.get_indexer(metadata_columns)
    if (metadata_indices < 0).any():
        missing = [
            column
            for column, index in zip(metadata_columns, metadata_indices)
            if index < 0
        ]
        raise ValueError(
            "Cleaned dataframe is missing encoded metadata column(s): {}.".format(
                ", ".join(missing)
            )
        )

    if preprocessor.dataset_config is not None:
        signal_columns = list(preprocessor.signal_columns)
        signal_indices = columns.get_indexer(signal_columns)
        if (signal_indices < 0).any():
            missing = [
                column
                for column, index in zip(signal_columns, signal_indices)
                if index < 0
            ]
            raise ValueError(
                "Cleaned dataframe is missing configured signal column(s): {}.".format(
                    ", ".join(missing)
                )
            )

        configured_indices = np.concatenate((signal_indices, metadata_indices))
        if len(np.unique(configured_indices)) != len(configured_indices):
            raise ValueError("Configured signal and metadata model columns overlap.")
        configured_index_set = set(configured_indices.tolist())
        if configured_index_set != set(range(len(columns))):
            undeclared = [
                column
                for index, column in enumerate(columns)
                if index not in configured_index_set
            ]
            raise ValueError(
                "Cleaned dataframe contains model column(s) not declared as signal or "
                "encoded metadata: {}.".format(", ".join(undeclared))
            )
    else:
        signal_indices = np.setdiff1d(
            np.arange(len(columns)), metadata_indices, assume_unique=True
        )

    return np.asarray(signal_indices), np.asarray(metadata_indices)
