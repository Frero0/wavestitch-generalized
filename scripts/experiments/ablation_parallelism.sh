...
do
  ...
  do
    "$python_bin" -m scripts.synthesis.synthesis_wavestitch_autoregressive_strided_preconditioning -d "$dataset" -synth_mask "$synth_mask" -stride 8
    "$python_bin" -m scripts.synthesis.synthesis_wavestitch_autoregressive_strided_preconditioning -d "$dataset" -synth_mask "$synth_mask" -stride 16
    "$python_bin" -m scripts.synthesis.synthesis_wavestitch_autoregressive_strided_preconditioning -d "$dataset" -synth_mask "$synth_mask" -stride 32
