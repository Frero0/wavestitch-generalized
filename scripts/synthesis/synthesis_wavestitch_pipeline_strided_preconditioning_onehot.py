import argparse
import torch
from wavestitch.data_utils import PreprocessorOneHot
...
if __name__ == "__main__":
    ...
    hierarchical_column_indices = df_synth.columns.get_indexer(preprocessor.hierarchical_features_onehot)
    ...
    for trial in range(args.n_trials):
            ...
            for idx, (test_batch, mask_batch) in enumerate(zip(test_dataloader, mask_dataloader)):
                ...
                for step in range(diffusion_config['T'] - 1, -1, -1):
                    ...
                    with torch.enable_grad():
                        ...
                        grad_xt = torch.autograd.grad(epsilon_pred, x, grad_outputs=torch.ones_like(epsilon_pred),
                                                      retain_graph=True)[0]
                        if step > 0:
                        ...
                        # loss2 = 0.0
                        loss1 = torch.sum((normal_denoising[:, :(args.window_size - args.stride),
        ...
        synth_df_reconverted_selected.to_csv(f'{path}synth_wavestitch_pipeline_stride_{args.stride}_trial_{trial}_onehot_grad_simplecoeff.csv')
        if trial == 0:
            with open(f'{path}denoiser_calls_pipeline_stride_{args.stride}_onehot_grad_simplecoeff.txt', 'w') as file:
                file.write(str(num_ops))

    with open(f'generated/{args.dataset}/{args.synth_mask}/denoiser_calls_pipeline_stride_{args.stride}_onehot_grad_simplecoeff.txt', 'a') as file:
