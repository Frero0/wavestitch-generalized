# import argparse
# import torch
# from wavestitch.data_utils import PreprocessorOrdinal
...
if __name__ == "__main__":
    ...
    for trial in range(args.n_trials):
        ...
        synth_df_reconverted_selected.to_csv(f'{path}synth_wavestitch_pipeline_stride_{args.stride}_trial_{trial}_ordinal_grad_simplecoeff.csv')
        if trial == 0:
            with open(f'{path}denoiser_calls_pipeline_stride_{args.stride}_ordinal_grad_simplecoeff.txt', 'w') as file:
                file.write(str(num_ops))

    with open(f'generated/{args.dataset}/{args.synth_mask}/denoiser_calls_pipeline_stride_{args.stride}_ordinal_grad_simplecoeff.txt', 'a') as file:
