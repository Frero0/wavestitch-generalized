import argparse
import torch
from wavestitch.training_utils import MyDataset, fetchModel, fetchDiffusionConfig
import numpy as np
from torch import from_numpy, optim, nn, randint, normal, sqrt, device, save
from torch.utils.data import DataLoader
import pandas as pd
import os
from wavestitch.metasynth import metaSynthTimeWeaver, metadataMask
...
if __name__ == "__main__":
    ...
    model1 = fetchModel(in_dim, out_dim, args).to(device)
    model2 = fetchModel(in_dim, out_dim, args).to(device)
    ...
    if args.propCycEnc:
        saved_params_signalconditioned = torch.load(
            f'saved_models/{args.dataset}/model_prop_sssd_signalconditioned.pth', map_location=device)
        saved_params = torch.load(f'saved_models/{args.dataset}/model_prop_sssd.pth', map_location=device)
    else:
        saved_params_signalconditioned = torch.load(f'saved_models/{args.dataset}/model_sssd_signalconditioned.pth',
                                                    map_location=device)
        saved_params = torch.load(f'saved_models/{args.dataset}/model_sssd.pth',
                                  map_location=device)
    with torch.no_grad():
        for name, param in model1.named_parameters():
            param.copy_(saved_params_signalconditioned[name])
    ...
    for trial in range(args.trials):
        ...
        with torch.no_grad():
            ...
            for idx, (test_batch, mask_batch) in enumerate(zip(test_dataloader, mask_dataloader)):
                ...
                metaonly_mask = torch.ones_like(conditional_mask).to(device)
                metaonly_mask[:, :, non_hier_cols] = 0
                ...
                for step in range(diffusion_config['T'] - 1, -1, -1):
                    ...
                    epsilon_pred = (1+3)*model1((x, test_batch, conditional_mask), times) - 3*model2((x, test_batch, metaonly_mask), times)  # 1+w * conditional - w * unconditional
                    epsilon_pred = epsilon_pred.permute((0, 2, 1))
                    if step > 0:
                        vari = beta_t * ((1 - alpha_bar_t_1) / (1 - alpha_bar_t)) * torch.normal(0, 1,
                                                                                                 size=epsilon_pred[:, :,
                                                                                                      non_hier_cols].shape).to(
                            device)
                    else:
                        vari = 0.0
                    ...
                    normal_denoising[:, :, non_hier_cols] = (x[:, :, non_hier_cols] - (
                            (beta_t / torch.sqrt(1 - alpha_bar_t)) * epsilon_pred[:, :, non_hier_cols])) / torch.sqrt(
                        alpha_t)
        ...
        if args.propCycEnc:
            synth_df_reconverted_selected.to_csv(f'{path}synth_timeautodiff_trial_{trial}_prop.csv')
        else:
            synth_df_reconverted_selected.to_csv(f'{path}synth_timeautodiff_trial_{trial}.csv')
